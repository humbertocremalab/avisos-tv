...
export default function DashboardPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      let uploadedImagenUrl = null;

      if ((tipo === "video" || tipo === "imagen") && file) {
        const publicUrl = await handleFileUpload(file);
        if (tipo === "video") uploadedUrl = publicUrl;
        if (tipo === "imagen") uploadedImagenUrl = publicUrl;
      }
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "'Poppins', sans-serif", background: "#f4f6f8", minHeight: "100vh", position: "relative" }}>
      <button onClick={handleLogout} style={{ padding: "10px 16px", background: "red", color: "#fff", border: "none", borderRadius: "8px", cursor: "pointer", position: "absolute", top: "20px", right: "20px", fontWeight: "600" }}>
        Cerrar sesión
      </button>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "12px", maxWidth: "420px", margin: "0 auto", padding: "20px", border: "1px solid #e0e0e0", borderRadius: "16px", background: "#fff", color: "#000", boxShadow: "0 4px 8px rgba(0,0,0,0.05)" }}>
        ...
        <input type="text" placeholder="Título (opcional)" value={titulo} onChange={(e) => setTitulo(e.target.value)} style={{ padding: "10px", fontSize: "1rem", borderRadius: "8px", border: "1px solid #ccc", color: "#000" }} />

        {tipo === "texto" && <textarea placeholder="Descripción" value={descripcion} onChange={(e) => setDescripcion(e.target.value)} style={{ padding: "10px", fontSize: "1rem", borderRadius: "8px", border: "1px solid #ccc", minHeight: "100px" }} required />}

        {(tipo === "video" || tipo === "imagen") && <input type="file" accept={tipo === "imagen" ? "image/*" : "video/*"} onChange={(e) => setFile(e.target.files[0])} style={{ marginTop: "10px" }} required />}
